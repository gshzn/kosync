...
class ClientGenerator:
    ...
    def _download_latest_nickelmenu_version(self) -> Path:
        path, _ = urlretrieve("https://github.com/pgaskin/NickelMenu/releases/download/v0.5.4/KoboRoot.tgz")
    ...
    def _prepare_client(self) -> None:
        ...
        if not client_path.exists():
            raise ValueError(f"No client executable found at the set path: {client_path.name}")
        
        shutil.copyfile(client_path, Path(self.root_directory) / "mnt" / "onboard" / "kosync_client")
        (Path(self.root_directory) / "mnt" / "onboard" / ".adds" / "nm" / "doc").write_text(self._nickelmenu_config())

    def generate(self, token: str) -> Path:
        config_path = Path(self.root_directory) / "mnt" / "onboard" / "kosync" / ".kosyncConfig.json"
        ...
        with tarfile.open(path, mode="w:gz") as tar_file:
            for subdir in ["usr", "mnt"]:
                tar_file.add(Path(self.root_directory) / subdir)
